(function($) {
  $.fn.richText = function(options) {
    ...
    function init() {
      ...
      if (settings.height && settings.height > 0) {
        ...
        $editor
          ...
          .css({ "min-height": height + "px", height: height + "px" });
      }
        ...
        if (!url) {
          ...
          if ($editNode.length > 0 && $editNode.prop("tagName") === "IMG") {
            ...
            if (
              ...
              $editNode.parent("div").attr("style") === "text-align:center;" &&
              align !== "center"
            ) {
              $editNode.unwrap("div");
    ...
    $(document).on("click", ".richText-toolbar a[data-command]", function(
      event
    ) {
      var $button = $(this);
      ...
      if (
        $editor.length > 0 &&
        id === editorID &&
        (!$button.parent("li").attr("data-disable") ||
          $button.parent("li").attr("data-disable") === "false")
      ) {
        event.preventDefault();
        ...
        if (command === "toggleCode") {
          ...
          if (command === "removeFormat") {
            ...
            $editor.find("*").each(function() {
              ...
              $.each(attributes, function(i, item) {
                if (
                  keepAttributes.indexOf(item) < 0 &&
                  item.substr(0, 5) !== "data-"
                ) {
          ...
          $editor
            .find(
              "div:empty,p:empty,li:empty,h1:empty,h2:empty,h3:empty,h4:empty,h5:empty,h6:empty"
            )
            .remove();
