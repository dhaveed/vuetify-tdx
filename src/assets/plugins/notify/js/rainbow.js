/* Rainbow v2.1.3 rainbowco.de | included languages: c, coffeescript, csharp, css, d, generic, go, haskell, html, java, javascript, json, lua, php, python, r, ruby, scheme, shell, smalltalk, sql */ !(function(
  e,
  t
) {
  ...
  function n(e) {
    var t =
      ...
      e.parentNode.getAttribute("data-language");
    if (!t) {
      var n = /\blang(?:uage)?-(\w+)/,
  ...
  function c(t, Prism) {
    ...
    var f = c + "\tthis.onmessage=" + t.toString(),
      l = new Blob([f], { type: "text/javascript" });
  ...
  function b(e, t) {
    ...
    for (
      var n = e.getElementsByTagName("pre"),
        r = e.getElementsByTagName("code"),
        a = [],
        o = [],
        i = 0,
        u = n;
      ...
      var s = u[i];
      h(s),
        s.getElementsByTagName("code").length
          ? s.getAttribute("data-trimmed") ||
            (s.setAttribute("data-trimmed", !0),
  ...
  var M,
    Prism = function Prism(e) {
      function t(e, t) {
        for (var n in h)
          if (
            ((n = parseInt(n, 10)),
            r(n, h[n], e, t) && (delete h[n], delete p[n]),
            i(n, h[n], e, t))
          )
            return !0;
        return !1;
      ...
      function l(r, a, i) {
        function c(e) {
          return (
            r.name && (e = n(r.name, e)),
            (p[y] = { replace: m[0], with: e }),
            (h[y] = w),
            !g && { remaining: a.substr(w - i), offset: w }
          );
        }
        function l(t) {
          var a = m[t];
        ...
        !r.name &&
          r.matches &&
          "string" == typeof r.matches[0] &&
          ((r.name = r.matches[0]), delete r.matches[0]);
        var b = m[0],
          y = m.index + i,
          w = m[0].length + y;
        if (y === w) return !1;
        if (t(y, w)) return { remaining: a.substr(w - i), offset: w };
      ...
      function g(t) {
        for (var n = e.patterns[t] || []; e.inheritenceMap[t]; )
    ...
    k = t(),
    j = null;
  (x = { extend: w, remove: N, onHighlight: y, addAlias: E, color: L }),
    R &&
      (x.colorSync = function(e, t) {
        var n = v(e, t),
          r = new Prism(n.options);
...
Rainbow.extend(
  ...
  [
    {
      ...
      matches: {
        1: [
          {
            matches: { 1: "keyword.define", 2: "entity.name" },
            pattern: /(\w+)\s(\w+)\b/g
          },
          { name: "keyword.define", pattern: /endif/g },
          { name: "constant.numeric", pattern: /\d+/g },
          {
            matches: { 1: "keyword.include", 2: "string" },
            pattern: /(include)\s(.*?)$/g
          }
        ]
      },
  ...
  Rainbow.extend("d", [
    { name: "constant", pattern: /\b(false|null|true)\b/gm },
    {
      name: "keyword",
    ...
    {
      matches: { 1: "keyword", 2: { name: "support.class", pattern: /\w+/gm } },
      pattern: /(typeof)\s([^\$].*?)(\)|;)/gm
    },
    {
      matches: {
        1: "keyword.namespace",
        2: { name: "support.namespace", pattern: /\w+/gm }
      },
    ...
    { name: "keyword.static", pattern: /\b(static)\b/gm },
    {
      matches: {
        1: "keyword.new",
        2: { name: "support.class", pattern: /\w+/gm }
      },
      pattern: /\b(new)\s([^\$].*?)(?=\)|\(|;|&)/gm
    },
    { name: "string", pattern: /("|')(.*?)\1/gm },
    { name: "integer", pattern: /\b(0x[\da-f]+|\d+)\b/gm },
    {
      name: "comment",
  ...
  Rainbow.extend(
    ...
    [
      {
        matches: {
          1: [{ name: "keyword.operator", pattern: /\=|\+/g }],
          2: {
            name: "string",
            matches: {
      ...
      { name: "keyword", pattern: /\bpackage(?=\s*\w)/gi },
      {
        matches: { 1: "storage.type", 2: "entity.name.struct" },
        pattern: /\b(type)\s+(\w+)\b(?=\s+struct\b)/gi
      },
      {
        matches: { 1: "storage.type", 2: "entity.name.type" },
      ...
      { name: "keyword.operator.initialize", pattern: /\:=/g },
      {
        matches: { 1: "storage.function", 2: "entity.name.function" },
        pattern: /(func)\s+(?:\(.*?\))\s+(.*?)(?=\()/g
      },
  ...
  Rainbow.extend(
    "haskell",
    [
      { name: "comment", pattern: /\{\-\-[\s\S(\w+)]+[\-\-][\}$]/gm },
      { name: "comment", pattern: /\-\-(.*)/g },
      {
        matches: { 1: "keyword", 2: "support.namespace" },
      ...
      { name: "keyword", pattern: /[\`][a-zA-Z_']*?[\`]/g },
      {
        matches: { 1: "keyword", 2: "keyword.operator" },
        pattern: /\b(infix|infixr|infixl)+\s\d+\s(\w+)*/g
      },
      { name: "entity.class", pattern: /\b([A-Z][A-Za-z0-9_']*)/g },
      {
        name: "meta.preprocessor",
        matches: {
          1: [
  ...
  Rainbow.extend("java", [
    ...
    { matches: { 1: "entity.function" }, pattern: /([^@\.\s]+)\(/g },
  ...
  Rainbow.extend(
    ...
    [
      ...
      {
        matches: { 1: "storage.type.accessor", 2: "entity.name.function" },
  ...
  Rainbow.extend(
    ...
    [
      ...
      { name: "variable.language.php-tag", pattern: /(&lt;\?(php)?|\?&gt;)/gi },
      {
        matches: {
          1: "keyword.namespace",
          2: { name: "support.namespace", pattern: /\w+/g }
      ...
      {
        matches: { 1: "storage.function", 2: "entity.name.function.magic" },
  ...
  Rainbow.extend(
    ...
    [
      ...
      {
        matches: { 1: "storage.function" },
        pattern: /[^a-zA-Z0-9._](function)(?=\s*\()/g
      },
      {
        matches: { 1: "namespace", 2: "keyword.operator", 3: "function.call" },
  ...
  Rainbow.extend("ruby", [
    ...
    { name: "constant.numeric", pattern: /\b(0x[\da-f]+|[\d_]+)\b/g },
  ...
  Rainbow.extend("scheme", [
    { name: "plain", pattern: /&gt;|&lt;/g },
    { name: "comment", pattern: /;.*$/gm },
    { name: "constant.language", pattern: /#t|#f|'\(\)/g },
    { name: "constant.symbol", pattern: /'[^()\s#]+/g },
    { name: "constant.number", pattern: /\b\d+(?:\.\d*)?\b/g },
    { name: "string", pattern: /".+?"/g },
    {
      matches: { 1: "storage.function", 2: "variable" },
  ...
  Rainbow.extend("smalltalk", [
    { name: "keyword.pseudovariable", pattern: /self|thisContext/g },
    { name: "keyword.constant", pattern: /false|nil|true/g },
    { name: "string", pattern: /'([^']|'')*'/g },
    { name: "string.symbol", pattern: /#\w+|#'([^']|'')*'/g },
    { name: "string.character", pattern: /\$\w+/g },
    { name: "comment", pattern: /"([^"]|"")*"/g },
    {
      name: "constant.numeric",
      pattern: /-?\d+(\.\d+)?((r-?|s)[A-Za-z0-9]+|e-?[0-9]+)?/g
    },
    { name: "entity.name.class", pattern: /\b[A-Z]\w*/g },
    { name: "entity.name.function", pattern: /\b[a-z]\w*:?/g },
    {
      name: "entity.name.binary",
      pattern: /(&lt;|&gt;|&amp;|[=~\|\\\/!@*\-_+])+/g
    },
    { name: "operator.delimiter", pattern: /;[\(\)\[\]\{\}]|#\[|#\(^\./g }
  ]),
  Rainbow.extend("sql", [
    {
      matches: {
    ...
    { name: "function.call", pattern: /(\w+?)(?=\()/g },
    {
      name: "keyword",
