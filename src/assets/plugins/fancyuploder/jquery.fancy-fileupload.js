...
(function($) {
  ...
  var GetDisplayFilesize = function(numbytes, adjustprecision, units) {
    ...
    return (
      (pos == 0 || (adjustprecision && result >= 99.995)
        ? result.toFixed(0)
        : result.toFixed(2)) +
      " " +
      abbreviations[pos]
  ...
  var DisplayPreviewDialog = function(
    preview,
    endelem,
    inforow,
    data,
    ...
    var previewbackground = $("<div>").addClass(
    ...
    var previewdialog = $("<div>")
      .addClass("ff_fileupload_dialog_main")
      .append(previewclone);
    ...
    var HidePreviewDialog = function() {
      ...
      if (settings.hidepreview)
        settings.hidepreview.call(inforow, data, preview, previewclone);
    };
    ...
    if (settings.showpreview)
      settings.showpreview.call(inforow, data, preview, previewclone);
  };

  var InitShowAriaLabelInfo = function(inforow) {
    inforow.find("button").hover(
      function() {
        var val = $(this).attr("aria-label");

        if (val) {
          inforow
            .find(".ff_fileupload_buttoninfo")
            .text(val)
            .removeClass("ff_fileupload_hidden");
          inforow
            .find(".ff_fileupload_fileinfo")
  ...
  $.fn.FancyFileUpload = function(options) {
    this.each(function() {
      ...
      if (
        ...
        typeof $this.data("fancy-fileupload") === "object"
      ) {
    ...
    $(document).off("drop.fancy_fileupload dragover.fancy_fileupload");
    $(document).on("drop.fancy_fileupload dragover.fancy_fileupload", function(
      e
    ) {
      e.preventDefault();
    });
    ...
    $(window).on("beforeunload.fancy_fileupload", function(e) {
      ...
      if (active.length || queued.length) {
        ...
        setTimeout(function() {
          active.addClass("ff_fileupload_bounce");
        }, 250);

        queued.removeClass("ff_fileupload_bounce");
        setTimeout(function() {
          queued.addClass("ff_fileupload_bounce");
        }, 250);
    ...
    var AddFile = function(uploads, e, data) {
      ...
      data.ff_info.displayfilesize = GetDisplayFilesize(
        data.files[0].size,
        settings.adjustprecision,
      ...
      var StartUpload = function(e) {
        ...
        inforow
          .find(".ff_fileupload_fileinfo")
          .text(
            data.ff_info.displayfilesize +
              " | " +
              Translate("Starting upload...")
          );

        // Display progress bar.
        inforow
          .find(".ff_fileupload_progress_background")
          .removeClass("ff_fileupload_hidden");

        // Alter remove buttons.
        inforow
          .find("button.ff_fileupload_remove_file")
          .attr("aria-label", Translate("Cancel upload and remove from list"));
        ...
        if (settings.startupload)
          settings.startupload.call(inforow, SubmitUpload, e, data);
        else SubmitUpload();
      };
      ...
      var RemoveFile = function(e) {
        ...
        if (inforow.hasClass("ff_fileupload_uploading")) {
          ...
          if (inforow.hasClass("ff_fileupload_starting")) {
            ...
            if (settings.uploadcancelled)
              settings.uploadcancelled.call(data.ff_info.inforow, e, data);
          }
      ...
      if (URL && URL.createObjectURL) {
        ...
        if (url) {
          if (
            ...
            inforow
              .find(".ff_fileupload_preview_image")
            ...
            data.files[0].type.lastIndexOf("audio/", 0) > -1 &&
            audioelem.canPlayType &&
            audioelem.canPlayType(data.files[0].type)
          ) {
            haspreview = true;
            preview = $("<audio>")
              .attr("src", url)
            ...
            data.files[0].type.lastIndexOf("video/", 0) > -1 &&
            videoelem.canPlayType &&
            videoelem.canPlayType(data.files[0].type)
          ) {
            haspreview = true;
            preview = $("<video>")
              .attr("src", url)
      ...
      if (haspreview) {
        inforow
          .find(".ff_fileupload_preview_image")
          .addClass("ff_fileupload_preview_image_has_preview")
          .attr("aria-label", Translate("Preview"))
          .click(function(e) {
            e.preventDefault();
        ...
        inforow
          .find(".ff_fileupload_preview_image")
          .prop("disabled", true)
          .attr("aria-label", Translate("No preview available"))
          .click(function(e) {
            e.preventDefault();
          });
      }
      ...
      if (!hasimage)
        inforow
          .find(".ff_fileupload_preview_image")
          .addClass("ff_fileupload_preview_text_with_color")
          .addClass("ff_fileupload_preview_text_" + fileextclass)
          .text(fileext);
      ...
      if (settings.accept) {
        ...
        for (var x = 0; x < settings.accept.length && !found; x++) {
          if (
            settings.accept[x] === fileext ||
            settings.accept[x] === data.files[0].type
          )
            found = true;
        }

        if (!found)
          data.ff_info.errors.push(Translate("Invalid file extension."));
      }

      if (
        settings.maxfilesize > -1 &&
        data.files[0].size > settings.maxfilesize
      )
        data.ff_info.errors.push(
          FormatStr(
            Translate("File is too large.  Maximum file size is {0}."),
            GetDisplayFilesize(
      ...
      if (settings.edit && !data.ff_info.errors.length) {
        inforow.find(".ff_fileupload_filename").append(
          $("<input>")
            .attr("type", "text")
            .val(filename)
            .keydown(function(e) {
      ...
      inforow
        ...
        .text(
          data.ff_info.displayfilesize +
            (hasimage && settings.edit && !data.ff_info.errors.length
              ? " | ." + fileext
              : "")
        );
      ...
      if (data.ff_info.errors.length)
        inforow
          .find(".ff_fileupload_errors")
          .html(data.ff_info.errors.join("<br>"))
          .removeClass("ff_fileupload_hidden");

      // Action buttons.
      if (!data.ff_info.errors.length) {
        inforow.find(".ff_fileupload_actions").append(
          $("<button>")
            .addClass("ff_fileupload_start_upload")
            .attr("type", "button")
            .attr("aria-label", Translate("Start uploading"))
            .click(StartUpload)
        );
        inforow.find(".ff_fileupload_actions_mobile").append(
          $("<button>")
            .addClass("ff_fileupload_start_upload")
            .attr("type", "button")
            .attr("aria-label", Translate("Start uploading"))
      ...
      inforow.find(".ff_fileupload_actions").append(
        $("<button>")
          .addClass("ff_fileupload_remove_file")
          .attr("type", "button")
          .attr("aria-label", Translate("Remove from list"))
          .click(RemoveFile)
      );
      inforow.find(".ff_fileupload_actions_mobile").append(
        $("<button>")
    ...
    var UploadProgress = function(e, data) {
      ...
      if (
        ...
        settings.continueupload.call(data.ff_info.inforow, e, data) === false
      )
        data.abort();
    };
    ...
    var UploadFailed = function(e, data) {
      ...
      if (
        ...
        data.ff_info.retries < settings.retries
      ) {
        data.ff_info.fileinfo.text(
          FormatStr(
            Translate("{0} | Network error, retrying in a moment... ({1})"),
            data.ff_info.displayfilesize,
            data.errorThrown
          )
        );
      ...
      if (data.ff_info.removewidget) {
        ...
        if (data.errorThrown === "abort")
          data.ff_info.errors.push(Translate("The upload was cancelled."));
        else if (data.errorThrown === "failed_with_msg")
          data.ff_info.errors.push(
            FormatStr(
              Translate("The upload failed.  {0} ({1})"),
              EscapeHTML(data.result.error),
              EscapeHTML(data.result.errorcode)
            )
          );
        else data.ff_info.errors.push(Translate("The upload failed."));
        data.ff_info.inforow
          .find(".ff_fileupload_errors")
          .html(data.ff_info.errors.join("<br>"))
          .removeClass("ff_fileupload_hidden");
    ...
    var UploadDone = function(e, data) {
      if (!data.result.success) {
        if (typeof data.result.error !== "string")
          data.result.error = Translate(
            "The server indicated that the upload was not successful.  No additional information available."
          );
        if (typeof data.result.errorcode !== "string")
          data.result.errorcode = "server_response";
      ...
      if (settings.uploadcompleted)
        settings.uploadcompleted.call(data.ff_info.inforow, e, data);
      ...
      if (data.ff_info.removewidget) {
        ...
        data.ff_info.inforow
          .find(".ff_fileupload_fileinfo")
          .text(
            data.ff_info.displayfilesize + " | " + Translate("Upload completed")
          );
    ...
    var UploadChunkSend = function(e, data) {
      if (data.ff_info) {
        ...
        if (data.ff_info.lastresult && !data.ff_info.lastresult.success) {
          ...
          if (typeof data.ff_info.lastresult.error !== "string")
            data.ff_info.lastresult.error = Translate(
              "The server indicated that the upload was not successful.  No additional information available."
            );
          if (typeof data.ff_info.lastresult.errorcode !== "string")
            data.ff_info.lastresult.errorcode = "server_response";
    ...
    return this.each(function() {
      ...
      if ($this.attr("accept")) {
        ...
        if (!settings.accept) {
          ...
          for (var x = 0; x < accept.length; x++) {
            ...
            settings.accept.push(
              opt.indexOf("/") < 0 && opt.lastIndexOf(".") > -1
                ? opt.substring(opt.lastIndexOf(".") + 1)
                : opt
            );
          }
        }
      ...
      var dropzone = $("<button>")
        ...
        .attr(
          "aria-label",
          Translate("Browse, drag-and-drop, or paste files to upload")
        );
      ...
      if (
        ...
        var audiobutton = $("<button>")
          .addClass("ff_fileupload_dropzone_tool")
          .addClass("ff_fileupload_recordaudio")
          .attr("type", "button")
          .attr("aria-label", Translate("Record audio using a microphone"));
              ...
              .then(function(stream) {
                ...
                audiorec.addEventListener("dataavailable", function(e) {
                  ...
                  if (audiorec.state === "inactive") {
                    ...
                    blob.lastModified = Math.floor(
                      blob.lastModifiedDate.getTime() / 1000
                    );
                    blob.name = FormatStr(
                      Translate("Audio recording - {0}.mp3"),
                      blob.lastModifiedDate.toLocaleString()
                    );
      ...
      if (
        ...
        var videobutton = $("<button>")
          .addClass("ff_fileupload_dropzone_tool")
          .addClass("ff_fileupload_recordvideo")
          .attr("type", "button")
          .attr("aria-label", Translate("Record video using a camera"));
        dropzonetools.append(videobutton);

        var videorecpreview = $("<video>")
          .prop("muted", true)
          .prop("autoplay", true)
          .addClass("ff_fileupload_recordvideo_preview")
          .addClass("ff_fileupload_hidden");
            ...
            var streamhandler = function(stream) {
              ...
              videorec.addEventListener("dataavailable", function(e) {
                ...
                if (videorec.state === "inactive") {
                  ...
                  blob.name = FormatStr(
                    Translate("Video recording - {0}.mp4"),
                    blob.lastModifiedDate.toLocaleString()
                  );
                  ...
                  if (videorecpreview[0].src !== "")
                    videorecpreview[0].src = "";
              ...
              try {
                videorecpreview[0].src = URL.createObjectURL(stream);
              } catch {
                videorecpreview[0].srcObject = stream;
              }
      ...
      fileinput.fileupload(
        $.extend(baseoptions, settings.fileupload, immutableoptions)
      );
