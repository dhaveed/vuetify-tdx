...
(function($) {
  ...
  $("#autocomplete-dynamic").autocomplete({
    ...
    params: {
      action: "data_fetch",
      searh_type: function() {
        return $("select#autocomplete-select")
          .find("option:selected")
          .val();
      }
    },
  ...
  $(".delete_ad").on("click", function() {
    ...
    $.confirm({
      ...
      buttons: {
        confirm: function() {
          $("#sb_loading").show();
          $.post(carspot_ajax_url, { action: "sb_remove_ad", ad_id: id }).done(
            function(response) {
              $("#sb_loading").hide();
              var get_r = response.split("|");
              if ($.trim(get_r[0]) == "1") {
    ...
    setInterval(function() {
      $.post(carspot_ajax_url, {
        ...
        if ($.trim(get_r[0]) == "1") {
          ...
          $.post(carspot_ajax_url, { action: "sb_get_notifications" }).done(
  ...
  function getCustomTemplate(ajax_url, catId, updateId, is_top) {
    ...
    $.post(ajax_url, {
      ...
      if (is_category_based == 1) {
        if (is_top) {
          $.post(carspot_ajax_url, { action: "sb_get_car_total" }).done(
            function(cartTotal) {
