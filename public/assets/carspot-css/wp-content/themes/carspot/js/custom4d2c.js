...
(function ($) {
    ...
    $(window).on('load', function () {
        $('#ask_for_user_type').modal({backdrop: 'static', keyboard: false});
    });
    ...
    if ($('#save_user_type_old_users').length > 0)
    {
        $('#save_user_type_old_users').parsley().on('field:validated', function () {
                ...
                .on('form:submit', function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'save_user_type_old_users', dealer_contact_data: $("form#save_user_type_old_users").serialize(), }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        var get_r = response.split('|');
                        if ($.trim(get_r[0]) == '1')
    ...
    if ($('#dealer-contact-form').length > 0)
    {
        $('#dealer-contact-form').parsley().on('field:validated', function () {
                ...
                .on('form:submit', function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'dealer_contact', dealer_contact_data: $("form#dealer-contact-form").serialize(), }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        var get_r = response.split('|');
                        if ($.trim(get_r[0]) == '1')
    ...
    $('#autocomplete-dynamic').autocomplete({
        ...
        id: 'test',
        type: 'POST',
        params: {"action": "data_fetch", searh_type: function () {
        ...
        if (sb_action != "")
        {
            ...
            $.post(carspot_ajax_url, {action: sb_action}).done(function (response)
            {
                ...
                if ($('#current_action').val() == 'sb_load_messages')
                {
                    ...
                    $.post(carspot_ajax_url, {action: 'sb_load_messages', ad_id: cid, }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        $('#carspot_res').html(response);
                        ...
                        $('#sb_loading').show();
                        $.post(carspot_ajax_url, {action: 'sb_get_messages', ad_id: cid, user_id: second_user, receiver: second_user, inbox: 'no'}).done(function (response)
                        {
...
// Update Profile
    ...
    $('#sb_user_profile_update').on('click', function () {
        ...
        $('#sb_update_profile').parsley().on('field:validated', function () {
        }).on('form:submit', function () {
    ...
    $("#carspot_delete_account").on("click", function ()
    {
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'carspot_delete_account', user_id: use_id, }).done(function (response)
                    {
    ...
    $(".remove_fav_ad").on("click", function ()
    {
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'sb_fav_remove_ad', ad_id: id, }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        var get_r = response.split('|');
                        if ($.trim(get_r[0]) == '1')
    ...
    $(".delete_ad").on("click", function ()
    {
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'sb_remove_ad', ad_id: id, }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        var get_r = response.split('|');
                        if ($.trim(get_r[0]) == '1')
    ...
    $(".ad_status_new li").on("click", function ()
    {
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $('#sb_loading').show();
                    $.post(carspot_ajax_url, {action: 'sb_update_ad_status', ad_id: id, status: ad_status, }).done(function (response)
                    {
                        $('#sb_loading').hide();
                        var get_r = response.split('|');
                        if ($.trim(get_r[0]) == '1')
            ...
            $(this).parsley().on('field:validated', function () {
                var ok = $('.parsley-error').length === 0;
                    ...
                    .on('form:submit', function () {
                        ...
                        $('#sb_loading').show();
                        $.post(carspot_ajax_url, {action: 'sb_reply_user_rating', cid: comment_id, reply_text: comment_reply_text, }).done(function (response)
                        {
                            $('#sb_loading').hide();
        ...
        $("#dropzone").dropzone({
            ...
            init: function () {
                ...
                $.post(carspot_ajax_url, {
                    ...
                    is_update: $('#is_update').val()}).done(function (data)
                {
                ...
                this.on("removedfile", function (file) {
                    ...
                    if (img_id != "")
                    {
                        ...
                        $.post(carspot_ajax_url, {
                            ...
                            img: img_id, is_update: $('#is_update').val(),
