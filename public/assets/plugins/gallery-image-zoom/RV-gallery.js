class Gallery {
  constructor(options) {
    ...
    jQuery(this.gallery).on("click", ".big-arrows > *:first-child", function(
      e
    ) {
      e.stopPropagation();
      t.changeImg("prev");
  ...
  calcGalleryHeight() {
    ...
    if (this.aspectRatio != "auto" && jQuery.isNumeric(this.aspectRatio)) {
      this.gallery
        .find(".g_img-cont")
        .css("height", galleryWidth / this.aspectRatio);
    }
  }
  ...
  countMoveRight() {
    if (this.itemCount > 1) {
      ...
      for (var i = 0; i < this.itemCount; i++) {
        ...
        this.fullSmallWidth +=
          this.item +
          parseInt(this.gallery.find(".g_s-img").css("marginRight")) +
          parseInt(this.gallery.find(".g_s-img").css("marginLeft"));
      }

      this.moveRight =
        (this.fullSmallWidth - this.g_navDivWidth) /
        (this.gallery.find(".g_s-img").length - 1);
    }
  }
  ...
  setLeftSmallImages() {
    this.gallery
      .find(".g_s-img")
      .css("left", "-" + this.agi * this.moveRight + "px");
  }

  loadActiveImg(cb) {
    let t = this;
    t.bigImageSrc = t.gallery
      .find(".g_s-img img")
      .eq(t.agi)
      .attr("src");
    ...
    if (this.transition == "fade") {
      if (t.gallery.find(".g_m-img").attr("src") !== "#") {
        t.gallery
          .find(".g_img-cont .g_m-img")
          .not(".cloned")
          .clone()
          .appendTo(t.gallery.find(".g_img-cont .g_m-img").parent())
        ...
        t.gallery
          ...
          .not(".cloned")
          .clone()
          .appendTo(t.gallery.find(".g_f-s .g_m-img").parent())
      ...
      t.gallery
        .find(".g_m-img")
        .not(".cloned")
        .attr("src", t.bigImageSrc)
        .hide()
        .fadeIn(t.transitionTime / 2);
      ...
      if (!t.slideDirection) {
        ...
        if (this.slideDirection === "left") {
          ...
          t.gallery
            .find(".g_m-img")
            .not(".cloned")
            .animate({ left: "0" }, t.transitionTime);
          t.gallery
            .find(".g_m-img.cloned")
            .animate({ left: "-100%" }, t.transitionTime, function() {
              jQuery(this).remove();
            });
        } else {
          t.gallery
  ...
  changeImg(e) {
    ...
    if (this.itemCount > 1) {
      if (e === "prev") {
        ...
        this.images = jQuery.map(this.gallery.find(".g_s-img img"), function(
          i
        ) {
          return i.src;
        });
        ...
        if (
          this.images.indexOf(this.imgsrc) > this.agi &&
  ...
  closeFullScreen() {
    this.gallery.find(".g_f-s-hidden").fadeOut(
      400,
      function() {
  ...
  renderGallery() {
    ...
    jQuery(
      `<div class="arrow-div prev">
                      ` +
        this.nav[0] +
        `
                  </div>`
    ).prependTo(this.gallery.find(".g_nav-cont"));
    if (!this.showNav) {
      this.gallery.find(".g_nav-cont").hide();
    }
    jQuery(
      `<div class="arrow-div next">
                      ` +
        this.nav[1] +
        `
                  </div>`
    ).appendTo(this.gallery.find(".g_nav-cont"));
    ...
    if (this.arrows && this.itemCount > 1) {
      jQuery(
        `<div class="big-arrows"><div>` +
          this.nav[0] +
          `</div><div>` +
          this.nav[1] +
          `</div></div>`
      ).appendTo(this.gallery.find(".g_img-cont"));
    }

    if ((this.showDots && this.itemCount > 1) || this.showDotsIfOneItem) {
      jQuery(`<div class="dots"></div>`).appendTo(
        this.gallery.find(".g_img-cont")
      );
      for (let i = 0; i < this.itemCount; i++) {
        jQuery(`<div class="dot" data-item="` + i + `"></div>`).appendTo(
          this.gallery.find(".dots")
        );
      }
    }
