...
(function($, window, document, undefined) {
  ...
  AxGmap.prototype = {
    ...
    createLatLng: function(element) {
      var lat = 0;
      var lng = 0;
      var latlng = element.data("latlng")
        ? element.data("latlng")
        : this.element.data("latlng");
      if (latlng) {
    ...
    showMapStatus: function() {
      ...
      var status = $(
        '<div style="color:#000; background-color:#fff; border:solid 1px #ccc; width:' +
          self.element.width() +
          'px"><dl style="margin:1em;"><dt>Center LatLng</dt><dd class="axgmap-status-latlng"></dd><dt>Zoom</dt><dd class="axgmap-status-zoom"></dd><dt>Right Click LatLng</dt><dd class="axgmap-status-rightclick">none</dd></dl></div>'
      );
      status.insertAfter(this.element);
      google.maps.event.addListener(this.gmap, "idle", function() {
        $(".axgmap-status-latlng", status)
          .empty()
          .append(
            self.gmap
              .getCenter()
        ...
        $(".axgmap-status-zoom", status)
          .empty()
          .append(self.gmap.getZoom());
      });
      google.maps.event.addListener(this.gmap, "rightclick", function(event) {
        $(".axgmap-status-rightclick", status)
          .empty()
          .append(
