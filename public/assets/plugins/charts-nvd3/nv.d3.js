...
(function() {
  ...
  nv.deprecated = function(name, info) {
    if (console && console.warn) {
      console.warn(
        "nvd3 warning: `" + name + "` has been deprecated. ",
        info || ""
      );
    }
  };
        ...
        function mouseHandler() {
          ...
          if (scaleIsOrdinal) {
            ...
            if (xScale.range()[elementIndex] + xScale.rangeBand() >= mouseX) {
              pointXValue = xScale.domain()[
                d3.bisect(xScale.range(), mouseX) - 1
              ];
            } else {
              dispatch.elementMouseout({
                mouseX: mouseX,
    ...
    var positionTooltip = function() {
      nv.dom.read(function() {
        ...
        if (hidden) {
          ...
          var translateInterpolator = d3.interpolateString(
            old_translate,
            new_translate
          );
...
*/
  nv.utils.calcApproxTextWidth = function(svgTextElem) {
    if (
      nv.utils.isFunction(svgTextElem.style) &&
      nv.utils.isFunction(svgTextElem.text)
    ) {
      var fontSize = parseInt(
        svgTextElem.style("font-size").replace("px", ""),
        10
      );
...
*/
  nv.utils.inheritOptions = function(target, source) {
    ...
    target._inherited = nv.utils.arrayUnique(
      ops
        .concat(calls)
        .concat(inherited)
        .concat(ops)
        .concat(target._inherited || [])
    );
    target._d3options = nv.utils.arrayUnique(
      d3ops.concat(target._d3options || [])
    );
  };
...
*/
  nv.utils.availableHeight = function(height, container, margin) {
    return Math.max(
      0,
      nv.utils.sanitizeHeight(height, container) - margin.top - margin.bottom
    );
  };
...
*/
  nv.utils.availableWidth = function(width, container, margin) {
    return Math.max(
      0,
      nv.utils.sanitizeWidth(width, container) - margin.left - margin.right
    );
  };
 ...
 */
  ...
  nv.models.axis = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        switch (axis.orient()) {
          case "top":
            ...
            if (scale.range().length === 1) {
              ...
              w = isOrdinal
                ? scale.range()[0] + scale.range()[1] + scale.rangeBand()
                : scale.range()[1];
            } else if (scale.range().length > 2) {
              w =
            ...
            if (showMaxMin) {
              ...
              axisMaxMin
                .watchTransition(renderWatch, "min-max top")
                .attr("transform", function(d, i) {
                  return (
                    "translate(" + nv.utils.NaNtoZero(scale.range()[i]) + ",0)"
                  );
                });
            }
            break;
          case "bottom":
            ...
            if (showMaxMin) {
              ...
              axisMaxMin
                .attr("transform", function(d, i) {
                  return (
                    "translate(" +
                    nv.utils.NaNtoZero(
                      scale(d) + (isOrdinal ? scale.rangeBand() / 2 : 0)
                    ) +
                    ",0)"
                  );
                })
              ...
              axisMaxMin
                .watchTransition(renderWatch, "min-max bottom")
                .attr("transform", function(d, i) {
                  return (
                    "translate(" +
                    nv.utils.NaNtoZero(
          ...
          case "right":
            ...
            axisLabel
              ...
              .attr(
                "x",
                rotateYLabel ? d3.max(scale.range()) / 2 : axis.tickPadding()
              );
            if (showMaxMin) {
              axisMaxMin = wrap
                .selectAll("g.nv-axisMaxMin")
              ...
              axisMaxMin
                .watchTransition(renderWatch, "min-max right")
                .attr("transform", function(d, i) {
                  return (
                    "translate(0," + nv.utils.NaNtoZero(scale.range()[i]) + ")"
                  );
                })
                .select("text")
                .style("opacity", 1);
        ...
        if (
          showMaxMin &&
          (axis.orient() === "left" || axis.orient() === "right")
        ) {
          ...
          g.selectAll("g") // the g's wrapping each tick
            .each(function(d, i) {
              d3.select(this)
                ...
                .attr("opacity", 1);
              if (
                scale(d) < scale.range()[1] + 10 ||
                scale(d) > scale.range()[0] - 10
              ) {
          ...
          wrap.selectAll("g.nv-axisMaxMin").each(function(d, i) {
            try {
              if (i)
                // i== 1, max position
                maxMinRange.push(
                  scale(d) - this.getBoundingClientRect().width - 4
                );
              ...
              //assuming the max and min labels are as wide as the next tick (with an extra 4 pixels just in case)
              // i==0, min position
              else maxMinRange.push(scale(d) + 4);
            }
          });
        ...
        g.selectAll(".tick")
          .filter(function(d) {
            /*
                    ...
                    */
            return (
              !parseFloat(Math.round(d * 100000) / 1000000) && d !== undefined
            );
          })
          .classed("zero", true);
      ...
      selection.each(function(data) {
        ...
        boxplots
          ...
          .attr("transform", function(d, i, j) {
            return (
              "translate(" +
              (xScale(getX(d, i)) + xScale.rangeBand() * 0.05) +
              ", 0)"
            );
          })
        ...
        var box_width = function() {
          return maxBoxWidth === null
            ? xScale.rangeBand() * 0.9
            : Math.min(75, xScale.rangeBand() * 0.9);
        };
        ...
        [getWl, getWh].forEach(function(f) {
          ...
          boxplots
            .select("line.nv-boxplot-whisker.nv-boxplot-" + key)
            .watchTransition(renderWatch, "nv-boxplot: boxplots")
          ...
          boxplots
            .select("line.nv-boxplot-tick.nv-boxplot-" + key)
            .watchTransition(renderWatch, "nv-boxplot: boxplots")
            .attr("x1", box_left)
            .attr("y1", function(d, i) {
              return yScale(f(d));
        ...
        // outliers
        var outliers = boxplots
          .selectAll(".nv-boxplot-outlier")
          .data(function(d) {
            return getOlItems(d) || [];
          });
        outliers
  ...
  nv.models.boxPlotChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        nv.utils.initSVG(container);
        var availableWidth =
          (width || parseInt(container.style("width")) || 960) -
          margin.left -
          margin.right;
        var availableHeight =
        ...
        var barsWrap = g.select(".nv-barsWrap").datum(
    ...
    function chart(selection) {
      selection.each(function(d, i) {
        ...
        for (var i = 0, il = rangez.length; i < il; i++) {
          ...
          if (i <= 2) {
            rangeClassNames =
              rangeClassNames + " nv-range" + legacyRangeClassNames[i];
          }
        ...
        g.select("rect.nv-measure")
          ...
          .attr(
            "width",
            measurez < 0 ? x1(0) - x1(measurez[0]) : x1(measurez[0]) - x1(0)
          )
          .attr("x", xp1(measurez));
    ...
    function chart(selection) {
      selection.each(function(d, i) {
        ...
        var title = gEnter
          ...
          .attr(
            "transform",
            "translate(-6," + (height - margin.top - margin.bottom) / 2 + ")"
          );
        title
          .append("text")
          .attr("class", "nv-title")
  ...
  nv.models.candlestickBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        if (padData)
          x.range(
            xRange || [
              ...
              (availableWidth * (data[0].values.length - 0.5)) /
                data[0].values.length
            ]
          );
        else
          x.range(
            xRange || [5 + barWidth / 2, availableWidth - barWidth / 2 - 5]
          );
        ...
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-candlestickBar");
        ...
        wrap
          .select("#nv-chart-clip-path-" + id + " rect")
          .attr("width", availableWidth)
          .attr("height", availableHeight);

        g.attr(
          "clip-path",
          clipEdge ? "url(#nv-chart-clip-path-" + id + ")" : ""
        );
        ...
        var rects = tickGroups
          .append("rect")
          .attr("class", "nv-candlestick-rects nv-bars")
          .attr("transform", function(d, i) {
            return (
              "translate(" +
              (x(getX(d, i)) - barWidth / 2) +
              "," +
              (y(getY(d, i)) -
                (getOpen(d, i) > getClose(d, i)
                  ? y(getClose(d, i)) - y(getOpen(d, i))
                  : 0)) +
              ")"
            );
          })
    ...
    chart.highlightPoint = function(pointIndex, isHoverOver) {
      ...
      container
        .select(".nv-candlestickBar .nv-tick-0-" + pointIndex)
        .classed("hover", isHoverOver);
    };

    chart.clearHighlights = function() {
      container
        .select(".nv-candlestickBar .nv-tick.hover")
        .classed("hover", false);
    };
  ...
  nv.models.cumulativeLineChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        chart.update = function() {
          if (duration === 0) container.call(chart);
          else
            container
              .transition()
              .duration(duration)
        ...
        var wrap = container
          .selectAll("g.nv-wrap.nv-cumulativeLine")
          .data([data]);
        var gEnter = wrap
          .enter()
          .append("g")
        ...
        interactiveLayer.dispatch.on("elementMousemove", function(e) {
          ...
          var xValue = xAxis.tickFormat()(
            chart.x()(singlePoint, pointIndex),
            pointIndex
          );
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        bars
          ...
          .attr("width", (x.rangeBand() * 0.9) / data.length);
        bars
          .watchTransition(renderWatch, "discreteBar: bars")
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (showXAxis) {
          ...
          if (wrapLabels) {
            g.selectAll(".tick text").call(
              nv.utils.wrapTicks,
              chart.xAxis.rangeBand()
            );
          }
        }
  ...
  nv.models.distribution = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var distWrap = g.selectAll("g.nv-dist").data(
          function(d) {
            return d;
          },
          function(d) {
  ...
  nv.models.distroPlot = function() {
    ...
    var margin = { top: 0, right: 0, bottom: 0, left: 0 },
      ...
      getNl = function(d) {
        return (
          (centralTendency == "mean" ? getMean(d) : getQ2(d)) - d.values.notch
        );
      },
      getNu = function(d) {
        return (
    ...
    function select_sigma(x) {
      ...
      var normalize = 1.349;
      var IQR =
        (d3.quantile(sorted, 0.75) - d3.quantile(sorted, 0.25)) / normalize; // normalized IQR
    ...
    function calcBandwidth(x, type) {
      ...
      var n = x.length;
      return type === "scott"
        ? Math.pow(1.059 * A * n, -0.2)
        : Math.pow(0.9 * A * n, -0.2);
    }
    ...
    function prepData(dat) {
      ...
      function calcStats(g, xGroup) {
        ...
        if (typeof d3.beeswarm !== "undefined") {
          ...
          observations.map(function(e, i) {
            ...
            e.randX =
              Math.random() *
              jitter *
              (Math.floor(Math.random() * 2) == 1 ? 1 : -1); // calculate random x-position only once for each point
          });
        } else {
        ...
        var kde = kernelDensityEstimator(
          eKernel(bandwidthCalc),
          yScale.ticks(resolution)
        );
        var kdeDat = clampViolin
          ? clampViolinKDE(kde(v), d3.extent(v))
          : kde(v);

        // make a new vertical scale for each group
        var tmpScale = d3.scale
          .linear()
          .domain([
            0,
            d3.max(kdeDat, function(e) {
              return e.y;
            })
          ])
          .clamp(true);
    ...
    function isOutlier(d) {
      return (
        (whiskerDef == "iqr" && d.isOutlier) ||
        (whiskerDef == "stddev" && d.isOutlierStdDev)
      );
    }
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        // Setup x-scale
        xScale.rangeBands(xRange || [0, availableWidth], 0.1).domain(
          xDomain || (colorGroup && !squash)
            ? allColorGroups
            : reformatDat.map(function(d) {
                return d.key;
              })
        );
        ...
        var wrapEnter = wrap
          ...
          .attr("class", "nvd3 nv-wrap");
        wrap
          .watchTransition(renderWatch, "nv-wrap: wrap")
          .attr(
            "transform",
            "translate(" + margin.left + "," + margin.top + ")"
          );
        ...
        var areaEnter,
          distroplots = wrap
            .selectAll(".nv-distroplot-x-group")
            .data(function(d) {
              return d;
            });
        ...
        distroplots.each(function(d, i) {
          d3.select(this)
            .selectAll("line.nv-distroplot-middle")
            .datum(d);
        });

        areaEnter = distroplots
          .enter()
          .append("g")
        ...
        areaEnter.attr("transform", function(d) {
          return (
            "translate(" +
            (xScale(d.key) + (rangeBand() - areaWidth()) * 0.5) +
            ", 0)"
          );
        });
        ...
        [getWl, getWh].forEach(function(f) {
          ...
          var endpoint = f === getWl ? getQ1 : getQ3;
          distroplots
            .select("line.nv-distroplot-whisker.nv-distroplot-" + key)
            .watchTransition(renderWatch, "nv-distroplot-x-group: distroplots")
            .attr("x1", areaCenter())
            .attr("y1", function(d) {
          ...
          distroplots
            .select("line.nv-distroplot-tick.nv-distroplot-" + key)
            .watchTransition(renderWatch, "nv-distroplot-x-group: distroplots")
            .attr("x1", function(d) {
        ...
        [getWl, getWh].forEach(function(f) {
          var key = f === getWl ? "low" : "high";
          areaEnter
            .selectAll(".nv-distroplot-" + key)
            .on("mouseover", function(d, i, j) {
              d3.select(this.parentNode)
                .selectAll("line.nv-distroplot-" + key)
                .classed("hover", true);
              dispatch.elementMouseover({
        ...
        areaEnter.each(function(d, i) {
          ...
          ["left", "right"].forEach(function(side) {
            ["line", "area"].forEach(function(d) {
              violin
                ...
                .attr(
                  "class",
                  "nv-distribution-" + d + " nv-distribution-" + side
                )
                .attr(
                  "transform",
                  "rotate(90,0,0)   translate(0," +
                    (side == "left" ? -areaWidth() : 0) +
                    ")" +
                    (side == "left" ? "" : " scale(1,-1)")
                ); // rotate violin
            });
          ...
          areaEnter.selectAll(".nv-distribution-line").style("fill", "none");
          areaEnter
            .selectAll(".nv-distribution-area")
            .style("stroke", "none")
            .style("opacity", 0.7);
        });
        ...
        distroplots.each(function(d, i) {
          ...
          var objData =
            plotType == "box"
              ? makeNotchBox(areaLeft(), tickLeft(), areaCenter(), d)
              : d.values.kde;
          ...
          if (plotType == "box" || plotType == "violin") {
            ["left", "right"].forEach(function(side) {
              // line
              distroplots
                .selectAll(".nv-distribution-line.nv-distribution-" + side)
              ...
              // area
              distroplots
                .selectAll(".nv-distribution-area.nv-distribution-" + side)
                //.watchTransition(renderWatch, 'nv-distribution-line: distroplots') // disable transition for now because it's jaring
                .attr(
                  "d",
                  d3.svg
            ...
            distroplots
              .selectAll(".nv-distribution-line")
        ...
        distroplots
          ...
          .watchTransition(
            renderWatch,
            "nv-distroplot-x-group: distroplots line"
          )
          .attr(
            "x1",
            notchBox ? tickLeft : plotType != "violin" ? areaLeft : tickLeft()
          )
          .attr("y1", function(d, i, j) {
            return centralTendency == "mean"
              ? yScale(getMean(d))
              : yScale(getQ2(d));
          })
          .attr(
        ...
        // tooltip
        distroplots
          .selectAll(".nv-distroplot-middle")
          .on("mouseover", function(d, i, j) {
        ...
        var obsWrap = distroplots.selectAll("g.nv-distroplot-observation").data(
          function(d) {
            return getValsObj(d);
          },
          function(d) {
        ...
        var obsGroup = obsWrap
          .enter()
          .append("g")
          .attr("class", "nv-distroplot-observation");

        obsGroup.append("circle").style({ opacity: 0 });
        ...
        if (observationType == "line") {
          distroplots
            .selectAll("g.nv-distroplot-observation line")
            .watchTransition(
              renderWatch,
              "nv-distrolot-x-group: nv-distoplot-observation"
            )
          ...
          distroplots
            .selectAll("g.nv-distroplot-observation circle")
            .watchTransition(
              renderWatch,
              "nv-distroplot: nv-distroplot-observation"
            )
            .attr("cy", function(d) {
              return yScale(d.datum);
            })
          ...
          distroplots
            ...
            .attr("cx", function(d) {
              return observationType == "swarm"
                ? d.x + areaWidth() / 2
                : observationType == "random"
                ? areaWidth() / 2 + (d.randX * areaWidth()) / 2
                : areaWidth() / 2;
            });
        }
        ...
        if (observationType !== false) {
          // observationType is False when hidding all circle/lines
          if (!showOnlyOutliers) {
            // show all line/circle
            distroplots
              .selectAll(observationType == "line" ? "line" : "circle")
              .watchTransition(
                renderWatch,
                "nv-distroplot: nv-distroplot-observation"
              )
            ...
            distroplots
              .selectAll(
                ".nv-distroplot-outlier " +
                  (observationType == "line" ? "line" : "circle")
              )
              .watchTransition(
                renderWatch,
                "nv-distroplot: nv-distroplot-observation"
        ...
        distroplots
          .selectAll(
            ".nv-distroplot-observation" +
              (observationType == "line" ? " circle" : " line")
          )
          .watchTransition(
            renderWatch,
            "nv-distroplot: nv-distoplot-observation"
          )
          .style("opacity", 0);

        // tooltip events for observations
        distroplots
          .selectAll(".nv-distroplot-observation")
          .on("mouseover", function(d, i, j) {
            ...
            dispatch.elementMouseover({
              value:
                plotType == "box" && isOutlier(d) ? "Outlier" : "Observation",
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        chart.update = function() {
          ...
          if (
            colorGroup0 !== opts.colorGroup() || // recalc data when any of the axis accessors are changed
            x0 !== opts.x() ||
            y0 !== opts.y() ||
        ...
        if (
          typeof d3.beeswarm !== "function" &&
          chart.options().observationType() == "swarm"
        ) {
          var xPos = margin.left + availableWidth / 2;
          noData =
            'Please include the library https://github.com/Kcnarf/d3-beeswarm to use "swarm".';
          nv.utils.noData(chart, container);
          return chart;
        ...
        var gEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-distroPlot")
          .append("g");
        var defsEnter = gEnter.append("defs");
        var g = wrap.select("g");
        ...
        g.watchTransition(renderWatch, "nv-wrap: wrap").attr(
          "transform",
          "translate(" + margin.left + "," + margin.top + ")"
        );
  ...
  nv.models.focus = function(content) {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var contentWrap = g.select(".nv-contentWrap").datum(
          data.filter(function(d) {
            return !d.disabled;
          })
        );
      ...
      selection.each(function(data) {
        ...
        node
          ...
          .on("mouseover", function(evt) {
            container
              .select(
                ".nv-series-" + evt.seriesIndex + " .nv-distx-" + evt.pointIndex
              )
              .attr("y1", evt.py);
            container
              .select(
                ".nv-series-" + evt.seriesIndex + " .nv-disty-" + evt.pointIndex
              )
              .attr("x2", evt.px);
      ...
      selection.each(function(data) {
            ...
            if (updateState) {
              if (vers == "classic") {
                ...
                if (expanded) {
                  ...
                  d.userDisabled =
                    d.userDisabled == undefined ? !!d.disabled : d.userDisabled;
        ...
        if (align) {
          ...
          series.each(function(d, i) {
            var legendText;
            if (
              keyFormatter(getKey(d)) &&
              keyFormatter(getKey(d)).length > maxKeyLength
            ) {
              var trimmedKey = keyFormatter(getKey(d)).substring(
                0,
                maxKeyLength
              );
              legendText = d3
          ...
          while (
            ...
            seriesPerRow < seriesWidths.length
          ) {
          ...
          while (legendWidth > availableWidth && seriesPerRow > 1) {
            ...
            legendWidth = columnWidths.reduce(function(
              prev,
              cur,
              index,
              array
          ...
          series.attr("transform", function(d, i) {
            return (
              ...
              "," +
              (5 + Math.floor(i / seriesPerRow) * versPadding) +
              ")"
            );
          });
          ...
          height =
            margin.top +
            margin.bottom +
            Math.ceil(seriesWidths.length / seriesPerRow) * versPadding;
        } else {
          var ypos = 5,
            newxpos = 5,
            maxwidth = 0,
...
*/
  ...
  nv.models.heatMap = function() {
    ...
    function getHeatmapValues(data, axis) {
      ...
      data.forEach(function(cell, i) {
        if (axis == "row") {
          ...
          // if calculating stat over entire dataset
          if (!(0 in vals)) vals[0] = [];
    ...
    function cellColor(d) {
      ...
      return (cellsAreNumeric() && !isNaN(colorVal)) ||
        typeof colorVal !== "undefined"
        ? colorScale(colorVal)
        : missingDataColor;
    }
    ...
    function getColorDomain() {
      if (cellsAreNumeric()) {
        // if cell values are numeric
        return normalize
          ? d3.extent(prepedData, function(d) {
              return getNorm(d);
            })
          : d3.extent(uniqueColor);
      } else if (!cellsAreNumeric()) {
        // if cell values are ordinal
        return uniqueColor;
      }
    }
    ...
    function prepData(data) {
      ...
      var ix = 0,
        iy = 0; // use these indices to position cell in x & y direction
      var combo,
        idx = 0;
        ...
        if (!isArrayInArray(uniqueCells, combo)) {
          ...
          console.warn(
            "The row/column position " +
              ...
              "/" +
              valY +
              " has multiple values; ensure each cell has only a single value."
          );
        }
      });
    ...
    var cellValueLabel = function(d) {
      var val = !normalize
        ? cellValueFormat(getCellValue(d))
        : cellValueFormat(getNorm(d));
      return (cellsAreNumeric() && !isNaN(val)) || typeof val !== "undefined"
        ? val
        : missingDataLabel;
    };
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        cellHeight = cellAspectRatio
          ? cellWidth / cellAspectRatio
          : availableHeight / Object.keys(uniqueY).length;
        if (cellAspectRatio)
          availableHeight =
            cellHeight * Object.keys(uniqueY).length -
            margin.top -
            margin.bottom;
        ...
        // Setup Scales
        xScale
          .domain(xDomain || sortObjByVals(uniqueX))
          .rangeBands(xRange || [0, availableWidth - cellBorderWidth / 2]);
        yScale
          .domain(yDomain || sortObjByVals(uniqueY))
          .rangeBands(yRange || [0, availableHeight - cellBorderWidth / 2]);
        colorScale = cellsAreNumeric()
          ? d3.scale.quantize()
          : d3.scale.ordinal();
        colorScale
          .domain(colorDomain || getColorDomain())
        ...
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-heatMapWrap");
        wrapEnter.append("g").attr("class", "cellWrap");

        wrap
          .watchTransition(renderWatch, "nv-wrap: heatMapWrap")
          .attr(
            "transform",
            "translate(" + margin.left + "," + margin.top + ")"
          );
        ...
        var gridLinesV = wrap
          .select(".cellGrid")
          .selectAll(".gridLines.verticalGrid")
        ...
        gridLinesV.exit().remove();

        var gridLinesH = wrap
          .select(".cellGrid")
          .selectAll(".gridLines.horizontalGrid")
        ...
        var xMetaWrap = wrapEnter
          ...
          .attr("transform", function() {
            return (
              "translate(0," +
              (-xMetaHeight() - cellBorderWidth - metaOffset) +
              ")"
            );
          });
        ...
        var yMetaWrap = wrapEnter
          ...
          .attr("transform", function(d, i) {
            return (
              "translate(" +
              (-yMetaWidth() - cellBorderWidth - metaOffset) +
              ",0)"
            );
          });
        ...
        wrap
          ...
          .attr("x2", function(d) {
            return d == 0 || d == numHLines
              ? availableWidth
              : availableWidth - cellBorderWidth;
          })
          .attr("y1", function(d) {
        ...
        cellWrap
          .on("mouseover", function(d, i) {
            ...
            dispatch.elementMouseover({
              value:
                getKeyByValue(uniqueX, ix) + " & " + getKeyByValue(uniqueY, iy),
              series: {
          ...
          .on("mouseout", function(d, i) {
            ...
            // .nv-heatMap
            if (
              x + cellBorderWidth >= availableWidth ||
              y + cellBorderWidth >= availableHeight ||
              x < 0 ||
              y < 0
            ) {
              // remove all hover classes
        ...
        allMetaRect
          ...
          .on("mouseout", function(d, i) {
            ...
            var coordinates = d3.mouse(
              d3.select(isColMeta ? ".xMetaWrap" : ".yMetaWrap").node()
            );
            var x = coordinates[0];
            var y = coordinates[1];
  ...
  nv.models.heatMapChart = function() {
    ...
    function quantizeLegendValues() {
      ...
      if (typeof e.domain()[0] === "string") {
        // if color scale is ordinal

        legendVals = e.domain();
      } else {
        // if color scale is numeric
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        heatMap.width(availableWidth).height(availableHeight);

        var heatMapWrap = g.select(".nv-heatMap").datum(
          data.filter(function(d) {
            return !d.disabled;
          })
        );
        ...
        xTicks
          ...
          .style(
            "text-anchor",
            rotateLabels > 0 ? "start" : rotateLabels < 0 ? "end" : "middle"
          );
        ...
        if (alignXAxis == "bottom") {
          ...
          if (heatMap.xMeta() !== false) {
            // if showing x metadata
            var pos =
              availableHeight +
              heatMap.metaOffset() +
              heatMap.cellBorderWidth();
            g.select(".xMetaWrap")
              .watchTransition(renderWatch, "heatMap: xMetaWrap")
              .attr("transform", function(d, i) {
        ...
        if (alignYAxis == "right") {
          axisY.attr(
            "transform",
            "translate(" + (availableWidth - xPos) + ",0)"
          );
          if (heatMap.yMeta() !== false) {
            // if showing y meatdata
            var pos =
              availableWidth + heatMap.metaOffset() + heatMap.cellBorderWidth();
            g.select(".yMetaWrap")
              .watchTransition(renderWatch, "heatMap: yMetaWrap")
              .attr("transform", function(d, i) {
        ...
        legendWrap
          ...
          .attr(
            "transform",
            "translate(0," + (alignXAxis == "top" ? availableHeight : -30) + ")"
          ); // TODO: more intelligent offset (-30) when top aligning legend
  ...
  nv.models.historicalBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        y.domain(
          yDomain || d3.extent(data[0].values.map(getY).concat(forceY))
        ).range(yRange || [availableHeight, 0]);
        ...
        // Setup containers and skeleton of chart
        var wrap = container
          .selectAll("g.nv-wrap.nv-historicalBar-" + id)
          .data([data[0].values]);
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-historicalBar-" + id);
        var defsEnter = wrapEnter.append("defs");
        var gEnter = wrapEnter.append("g");
        var g = wrap.select("g");
        ...
        bars
          ...
          .attr("transform", function(d, i) {
            return (
              "translate(" +
              (x(getX(d, i)) -
                (availableWidth / data[0].values.length) * 0.45) +
              ",0)"
            );
          })
          .on("mouseover", function(d, i) {
        ...
        bars
          ...
          .attr("class", function(d, i, j) {
            return (
              (getY(d, i) < 0 ? "nv-bar negative" : "nv-bar positive") +
              " nv-bar-" +
              j +
              "-" +
              i
  ...
  nv.models.ohlcBarChart = function() {
    ...
    chart.interactiveLayer.tooltip.contentGenerator(function(data) {
      ...
      return (
        ...
        "<table>" +
        "<tr><td>open:</td><td>" +
        chart.yAxis.tickFormat()(d.open) +
        "</td></tr>" +
        "<tr><td>close:</td><td>" +
        chart.yAxis.tickFormat()(d.close) +
        "</td></tr>" +
        "<tr><td>high</td><td>" +
        chart.yAxis.tickFormat()(d.high) +
        "</td></tr>" +
        "<tr><td>low:</td><td>" +
        chart.yAxis.tickFormat()(d.low) +
        "</td></tr>" +
        "</table>"
      );
    });
  ...
  nv.models.line = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        wrap
          .select("#nv-edge-clip-" + scatter.id() + " rect")
          .attr("width", availableWidth)
          .attr("height", availableHeight > 0 ? availableHeight : 0);

        g.attr(
          "clip-path",
          clipEdge ? "url(#nv-edge-clip-" + scatter.id() + ")" : ""
        );
        scatterWrap.attr(
          "clip-path",
          clipEdge ? "url(#nv-edge-clip-" + scatter.id() + ")" : ""
        ...
        groups
          .watchTransition(renderWatch, "line: groups")
          .style("stroke-opacity", 1)
        ...
        areaPaths
          .watchTransition(renderWatch, "line: areaPaths")
          .attr("d", function(d) {
            return (
              d3.svg
                .area()
        ...
        linePaths.watchTransition(renderWatch, "line: linePaths").attr(
          "d",
          d3.svg
            .line()
            .interpolate(interpolate)
      ...
      selection.each(function(data) {
        ...
        if (!showLegend) {
          ...
          if (legendPosition === "bottom") {
            ...
            g.select(".nv-legendWrap").attr(
              "transform",
              "translate(0," + (availableHeight + xAxis.height()) + ")"
            );
        ...
        if (!focusEnable && focus.brush.extent() === null) {
          ...
          g.select(".nv-focusWrap")
            ...
            .attr(
              "transform",
              "translate(0," +
                (availableHeight + margin.bottom + focus.margin().top) +
                ")"
            )
            .call(focus);
          var extent = focus.brush.empty()
            ? focus.xDomain()
            : focus.brush.extent();
          if (extent !== null) {
            onBrush(extent);
            ...
            .forEach(function(series, i) {
              ...
              var currentValues = series.values.filter(function(d, i) {
                ...
                if (extent[0] <= extent[1]) {
                  ...
                  return (
                    lines.x()(d, i) >= extent[1] && lines.x()(d, i) <= extent[0]
                  );
                }
              });
      ...
      {},
      {
        ...
        focusMargin: {
          ...
          set: function(_) {
            ...
            focus.margin.right =
              _.right !== undefined ? _.right : focus.margin.right;
            focus.margin.bottom =
              _.bottom !== undefined ? _.bottom : focus.margin.bottom;
            focus.margin.left =
              _.left !== undefined ? _.left : focus.margin.left;
          }
        },
        margin: {
          get: function() {
  ...
  nv.models.linePlusBarChart = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        x2.domain(
          d3.extent(d3.merge(series1.concat(series2)), function(d) {
            return d.x;
          })
        ).range([0, availableWidth]);
        ...
        if (!showLegend) {
          ...
          g.select(".nv-legendWrap")
            .datum(
              data.map(function(series) {
                series.originalKey =
                  series.originalKey === undefined
                    ? series.key
                    : series.originalKey;
                if (switchYAxisOrder) {
                  series.key =
                    series.originalKey +
                    (series.bar ? legendRightAxisHint : legendLeftAxisHint);
                } else {
                  series.key =
                    series.originalKey +
                    (series.bar ? legendLeftAxisHint : legendRightAxisHint);
                }
                return series;
              })
            )
          ...
          if (!marginTop && legend.height() !== margin.top) {
            ...
            availableHeight1 =
              nv.utils.availableHeight(height, container, margin) - focusHeight;
          }

          g.select(".nv-legendWrap").attr(
            "transform",
            "translate(" + legendXPosition + "," + -margin.top + ")"
          );
        }
        ...
        g.select(".nv-context").style(
          "display",
          focusEnable ? "initial" : "none"
        );
        ...
        var lines2Wrap = g.select(".nv-context .nv-linesWrap").datum(
          allDisabled(dataLines)
            ? [{ values: [] }]
        ...
        if (focusShowAxisX) {
          ...
          g.select(".nv-context .nv-x.nv-axis").attr(
            "transform",
            "translate(0," + y3.range()[0] + ")"
          );
        ...
        function onBrush() {
          ...
          var focusBarsWrap = g.select(".nv-focus .nv-barsWrap").datum(
            !dataBars.length
              ? [{ values: [] }]
          ...
          var linesOpacity =
            dataLines.length && !allDisabled(dataLines) ? 1 : 0;
      ...
      selection.each(function(data) {
        ...
        var barsEnter = bars
          ...
          .attr("x", function(d, i, j) {
            return stacked && !data[j].nonStackable
              ? 0
          ...
          .attr("width", function(d, i, j) {
            return (
              x.rangeBand() /
              (stacked && !data[j].nonStackable ? 1 : data.length)
            );
          })
          .attr("transform", function(d, i) {
            return "translate(" + x(getX(d, i)) + ",0)";
            ...
            .attr("x", function(d, i, j) {
              ...
              if (data[j].nonStackable) {
                ...
                if (data.length !== nonStackableCount) {
                  width =
                    (data[j].nonStackableSeries * x.rangeBand()) /
                    (nonStackableCount * 2);
                }
              }
              return width;
      ...
      selection.each(function(data) {
        ...
        if (showXAxis) {
          ...
          if (staggerLabels) {
            ...
            g.selectAll(".nv-x.nv-axis .nv-axisMaxMin text").attr(
              "transform",
              function(d, i) {
                return getTranslate(
                  0,
                  i === 0 || totalInBetweenTicks % 2 !== 0
                    ? staggerDown
                    : staggerUp
                );
              }
            ...
            xTicks
              .filter(function(d, i) {
                return (
                  i %
                    Math.ceil(
                      data[0].values.length / (availableWidth / 100)
                    ) !==
                  0
                );
              })
  ...
  nv.models.multiBarHorizontal = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        else y.range(yRange || [0, availableWidth]);
        ...
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-multibarHorizontal");
        var defsEnter = wrapEnter.append("defs");
        var gEnter = wrapEnter.append("g");
        var g = wrap.select("g");
        ...
        groups
          .exit()
          .watchTransition(renderWatch, "multibarhorizontal: exit groups")
          .style("stroke-opacity", 1e-6)
          .style("fill-opacity", 1e-6)
          .remove();
        groups
        ...
        groups
          .watchTransition(renderWatch, "multibarhorizontal: groups")
          .style("stroke-opacity", 1)
          ...
          bars
            ...
            .text(function(d, i) {
              ...
              return (
                t +
                "+" +
                valueFormat(Math.abs(yerr[1])) +
                "-" +
                valueFormat(Math.abs(yerr[0]))
              );
            });
          bars
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var gEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-multiBarHorizontalChart")
          .append("g");
        var g = wrap.select("g");
    ...
    function chart(selection) {
      selection.each(function(data) {
          ...
          g.select(".legendWrap")
            .datum(
              data.map(function(series) {
                ...
                series.key =
                  series.originalKey +
                  (series.yAxis == 1 ? "" : legendRightAxisHint);
                return series;
              })
            )
            .call(legend);
        ...
        lines1
          ...
          .interpolate(interpolate)
          .color(
            color_array.filter(function(d, i) {
              return (
                !data[i].disabled &&
                data[i].yAxis == 1 &&
                data[i].type == "line"
        ...
        bars1
          ...
          .color(
            color_array.filter(function(d, i) {
              return (
                !data[i].disabled && data[i].yAxis == 1 && data[i].type == "bar"
              );
            })
          );
        bars2
          ...
          .color(
            color_array.filter(function(d, i) {
              return (
                !data[i].disabled && data[i].yAxis == 2 && data[i].type == "bar"
              );
            })
          );
        stack1
        ...
        var lines1Wrap = g.select(".lines1Wrap").datum(
          dataLines1.filter(function(d) {
            return !d.disabled;
          })
        );
        var scatters1Wrap = g.select(".scatters1Wrap").datum(
          dataScatters1.filter(function(d) {
            return !d.disabled;
          })
        );
        var bars1Wrap = g.select(".bars1Wrap").datum(
          dataBars1.filter(function(d) {
            return !d.disabled;
          })
        );
        var stack1Wrap = g.select(".stack1Wrap").datum(
          dataStack1.filter(function(d) {
            return !d.disabled;
          })
        );
        var lines2Wrap = g.select(".lines2Wrap").datum(
          dataLines2.filter(function(d) {
            return !d.disabled;
          })
        );
        var scatters2Wrap = g.select(".scatters2Wrap").datum(
          dataScatters2.filter(function(d) {
            return !d.disabled;
          })
        );
        var bars2Wrap = g.select(".bars2Wrap").datum(
          dataBars2.filter(function(d) {
            return !d.disabled;
          })
        );
        var stack2Wrap = g.select(".stack2Wrap").datum(
        ...
        if (bars1.stacked() && dataBars1.length) {
          var extraValue1BarStacked = dataBars1
            .filter(function(d) {
              return !d.disabled;
            })
            .map(function(a) {
          ...
          if (extraValue1BarStacked.length > 0)
            extraValue1BarStacked = extraValue1BarStacked.reduce(function(
              a,
              b
            ) {
        ...
        if (bars2.stacked() && dataBars2.length) {
          var extraValue2BarStacked = dataBars2
            .filter(function(d) {
              return !d.disabled;
            })
            .map(function(a) {
          ...
          if (extraValue2BarStacked.length > 0)
            extraValue2BarStacked = extraValue2BarStacked.reduce(function(
              a,
              b
            ) {
              return a.map(function(aVal, i) {
        ...
        yScale1
          .domain(
            yDomain1 ||
              d3.extent(
                d3.merge(series1).concat(extraValue1BarStacked),
                function(d) {
                  return d.y;
                }
              )
        ...
        yScale2
          .domain(
            yDomain2 ||
              d3.extent(
                d3.merge(series2).concat(extraValue2BarStacked),
                function(d) {
                  return d.y;
                }
              )
          ...
          interactiveLayer.dispatch.on("elementMousemove", function(e) {
            ...
            data
              ...
              .forEach(function(series, i) {
                ...
                var currentValues = series.values.filter(function(d, i) {
                  return (
                    chart.x()(d, i) >= extent[0] && chart.x()(d, i) <= extent[1]
                  );
                });
  ...
  nv.models.ohlcBar = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        else x.range(xRange || [5 + w / 2, availableWidth - w / 2 - 5]);
    ...
    chart.highlightPoint = function(pointIndex, isHoverOver) {
      chart.clearHighlights();
      container
        .select(".nv-ohlcBar .nv-tick-0-" + pointIndex)
        .classed("hover", isHoverOver);
    };
  ...
  nv.models.parallelCoordinates = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        // Setup Scales
        x.rangePoints([0, availableWidth], 1).domain(
          enabledDimensions.map(function(d) {
            return d.key;
          })
        );
        ...
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-parallelCoordinates");
        var gEnter = wrapEnter.append("g");
        var g = wrap.select("g");
        ...
        gEnter
          .append("g")
          .attr("class", "nv-parallelCoordinates missingValuesline");
        ...
        if (!isNaN(step)) {
          ...
          missingValueslineText = wrap
            .select(".missingValuesline")
            .selectAll("text")
            .data([undefinedValuesLabel]);
          missingValueslineText.append("text").data([undefinedValuesLabel]);
          missingValueslineText.enter().append("text");
          missingValueslineText.exit().remove();
          missingValueslineText
        ...
        foreground.on("mouseover", function(d, i) {
          d3.select(this)
            .classed("hover", true)
            .style("stroke-width", d.strokeWidth + 2 + "px")
            .style("stroke-opacity", 1);
          dispatch.elementMouseover({
            label: d.name,
        ...
        var dimensionsEnter = dimensions
          .enter()
          .append("g")
          .attr("class", "nv-parallelCoordinates dimension");
        ...
        foreground.style("display", function(d) {
          var isActive = actives.every(function(p, i) {
            if (
              (isNaN(d.values[p]) || isNaN(parseFloat(d.values[p]))) &&
              extents[i][0] == y[p].brush.y().domain()[0]
            ) {
              return true;
            }
            return (
              extents[i][0] <= d.values[p] &&
              d.values[p] <= extents[i][1] &&
              !isNaN(parseFloat(d.values[p]))
            );
          });
        ...
        function path(d) {
          return line(
            enabledDimensions.map(function(p) {
              //If value if missing, put the value on the missing value line
              if (
                isNaN(d.values[p.key]) ||
                isNaN(parseFloat(d.values[p.key])) ||
                displayMissingValuesline
              ) {
                ...
                if (axisWithUndefinedValues.indexOf(p.key) < 0) {
                  var newscale = d3.scale
                    .linear()
                    .domain([min, domain[1]])
                    .range([availableHeight - 12, range[1]]);
              ...
              if (missingValuesline !== undefined) {
                if (
                  axisWithUndefinedValues.length > 0 ||
                  displayMissingValuesline
                ) {
        ...
        function restoreBrush(visible) {
          filters.forEach(function(f) {
            ...
            if (f.hasOnlyNaN) {
              f.extent[1] =
                ((y[f.dimension].domain()[1] - brushDomain[0]) *
                  (f.extent[1] - f.extent[0])) /
                  (oldDomainMaxValue[f.dimension] - f.extent[0]) +
                brushDomain[0];
            }
            if (f.hasNaN) {
        ...
        function brushend() {
          ...
          filters.forEach(function(f) {
            if (
              f.extent[0] === y[f.dimension].brush.y().domain()[0] &&
              axisWithUndefinedValues.indexOf(f.dimension) >= 0
            )
              f.hasNaN = true;
        ...
        function dragMove(d) {
          dragging[d.key] = Math.min(
            availableWidth,
            Math.max(0, (this.parentNode.__origin__ += d3.event.x))
          );
  ...
  nv.models.parallelCoordinatesChart = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        state
          .setter(stateSetter(dimensionData), chart.update)
        ...
        dimensionData.forEach(function(d, i) {
          d.originalPosition = isNaN(d.originalPosition)
            ? i
            : d.originalPosition;
        ...
        var parallelCoordinatesWrap = g
          .select(".nv-parallelCoordinatesWrap ")
          .datum(data);
        ...
        parallelCoordinates.dispatch.on("brushEnd", function(
          active,
          hasActiveBrush
        ) {
          if (hasActiveBrush) {
      ...
      selection.each(function(data) {
        ...
        if (donut && title) {
          ...
          wrap
            ...
            .style(
              "font-size",
              (Math.min(availableWidth, availableHeight) * donutRatio * 2) /
                (title.length + 2) +
                "px"
            )
          ...
          if (hideOverlapLabels) {
            pieLabels
              .each(function(d, i) {
                ...
                d.visible =
                  nv.utils.pointIsInArc(topLeft, d, arc) &&
      ...
      {},
      {
        ...
        margin: {
          ...
          set: function(_) {
            margin.top = typeof _.top != "undefined" ? _.top : margin.top;
            margin.right =
              typeof _.right != "undefined" ? _.right : margin.right;
            margin.bottom =
              typeof _.bottom != "undefined" ? _.bottom : margin.bottom;
            margin.left = typeof _.left != "undefined" ? _.left : margin.left;
          }
        },
        duration: {
  ...
  nv.models.sankeyChart = function() {
    ...
    function chart(selection) {
      selection.each(function(data) {
        ...
        if (!isDataValid) {
          console.error(
            "NVD3 Sankey chart error:",
            "invalid data format for",
            data
          );
          console.info(
            "Valid data format is: ",
            testData,
    ...
    chart._options = Object.create(
      {},
      {
        ...
        nodeStyle: {
          ...
          set: function(_) {
            nodeFillColor =
              _.fillColor !== undefined ? _.fillColor : nodeFillColor;
            nodeStrokeColor =
              _.strokeColor !== undefined ? _.strokeColor : nodeStrokeColor;
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (padData && data[0])
          x.range(
            xRange || [
              (availableWidth * padDataOuter + availableWidth) /
                (2 * data[0].values.length),
              availableWidth -
                (availableWidth * (1 + padDataOuter)) /
                  (2 * data[0].values.length)
            ]
          );
        ...
        // If scale's domain don't have a range, slightly adjust to make one... so a chart can show a single data point
        singlePoint =
          x.domain()[0] === x.domain()[1] || y.domain()[0] === y.domain()[1];
        ...
        yDom = yDom || [];
        domainDiff =
          domainDiff || yDom[0] !== y.domain()[0] || yDom[1] !== y.domain()[1];
        yDom = y.domain();
        ...
        var wrapEnter = wrap
          .enter()
          .append("g")
          .attr("class", "nvd3 nv-wrap nv-scatter nv-chart-" + id);
        var defsEnter = wrapEnter.append("defs");
        var gEnter = wrapEnter.append("g");
        var g = wrap.select("g");
        ...
        function updateInteractiveLayer() {
          ...
          if (useVoronoi === true) {
              ...
              data.map(function(group, groupIndex) {
                return group.values
                  .map(function(point, pointIndex) {
                    ...
                    return [
                      nv.utils.NaNtoZero(x(pX)) + Math.random() * 1e-4,
            ...
            for (var i = 0; i < vertices.length - 1; ) {
              if (
                Math.abs(vertices[i][0] - vertices[i + 1][0]) < epsilon &&
            ...
            var vPointPaths = pointPaths
              ...
              .append("svg:path")
              .attr("d", function(d) {
                if (!d || !d.data || d.data.length === 0) return "M 0 0";
                else return "M" + d.data.join(",") + "Z";
              })
            ...
            if (clipVoronoi) {
              ...
              wrap
                ...
                .selectAll("*")
                .remove(); // must do * since it has sub-dom
              var pointClips = wrap
                .select(".nv-point-clips")
                .selectAll("clipPath")
                .data(vertices);
            ...
            var mouseEventCallback = function(el, d, mDispatch) {
              ...
              var pos = {
                left:
                  x(getX(point, d.point)) +
                  box.left +
                  scrollLeft +
                  margin.left +
                  10,
                top:
                  y(getY(point, d.point)) +
                  box.top +
                  scrollTop +
                  margin.top +
                  10
        ...
        groups
          .watchTransition(renderWatch, "scatter: groups")
          .style("fill", function(d, i) {
            return color(d, i);
          })
          .style("stroke", function(d, i) {
        ...
        points
          ...
          .attr("transform", function(d) {
            return (
              "translate(" +
              nv.utils.NaNtoZero(x0(getX(d[0], d[1]))) +
              "," +
        ...
        points
          .filter(function(d) {
            // getDiffs must always be called to update cache
            return (
              getDiffs(d, "shape", getShape, "size", getSize) ||
              scaleDiff ||
              sizeDiff ||
              domainDiff
            );
        ...
        if (showLabels) {
          ...
          titles
            ...
            .attr("transform", function(d) {
              var dx =
                nv.utils.NaNtoZero(x0(getX(d[0], d[1]))) +
                Math.sqrt(z(getSize(d[0], d[1])) / Math.PI) +
                2;
              return (
                "translate(" +
                dx +
          ...
          titles
            .watchTransition(renderWatch, "scatter labels")
            .text(function(d, i) {
              return d[0].label;
            })
            .attr("transform", function(d) {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        var wrapEnter = wrap
          ...
          .attr(
            "class",
            "nvd3 nv-wrap nv-scatterChart nv-chart-" + scatter.id()
          );
        var gEnter = wrapEnter.append("g");
        var g = wrap.select("g");
        ...
        regLine
          ...
          .style("stroke-opacity", function(d, i) {
            return d.disabled ||
              typeof d.slope === "undefined" ||
              typeof d.intercept === "undefined"
              ? 0
              : 1;
          });
        ...
        g.select(".nv-distributionY")
          .attr(
            "transform",
            "translate(" +
              (rightAlignYAxis ? availableWidth : -distY.size()) +
              ",0)"
          )
          .datum(
            data.filter(function(d) {
  ...
  nv.models.sparkline = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        // Setup Scales
        x.domain(xDomain || d3.extent(data, getX)).range(
          xRange || [0, availableWidth]
        );

        y.domain(yDomain || d3.extent(data, getY)).range(
          yRange || [availableHeight, 0]
        );
      ...
      selection.each(function(data) {
        ...
        if (showLastValue) {
          ...
          value
            ...
            .style(
              "fill",
              sparkline.color()(data[data.length - 1], data.length - 1)
            )
        ...
        function updateValueLine() {
          ...
          g.select(".nv-hoverValue .nv-xValue").text(
            xTickFormat(sparkline.x()(data[index[0]], index[0]))
          );
          ...
          g.select(".nv-hoverValue .nv-yValue").text(
            yTickFormat(sparkline.y()(data[index[0]], index[0]))
          );
        }
  ...
  nv.models.stackedArea = function() {
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        path
          .watchTransition(renderWatch, "stackedArea path")
          .attr("d", function(d, i) {
        ...
        scatter.dispatch.on("elementMouseover.area", function(e) {
          g.select(".nv-chart-" + id + " .nv-area-" + e.seriesIndex).classed(
            "hover",
            true
          );
        });
    ...
    function chart(selection) {
      ...
      selection.each(function(data) {
        ...
        if (!showLegend) {
          ...
          var legendWidth =
            showControls && legendPosition === "top"
        ...
        if (!showControls) {
          ...
          var requiredTop = Math.max(
            controls.height(),
            showLegend && legendPosition === "top" ? legend.height() : 0
          );
        ...
        var stackedWrap = g.select(".nv-focus .nv-stackedWrap").datum(
          data.filter(function(d) {
            return !d.disabled;
          })
        );
            ...
            data
              ...
              .map(function(d, i) {
                return {
                  ...
                  values: d.values.filter(function(d, i) {
                    return (
                      stacked.x()(d, i) >= extent[0] &&
                      stacked.x()(d, i) <= extent[1]
                    );
                  }),
                  disableTooltip: d.disableTooltip
                };
        ...
        if (!wrap[0][0]) {
          wrap = container
            ...
            .attr(
              "transform",
              "translate(" +
                (availableWidth / 2 + margin.left + margin.right) +
                "," +
                (availableHeight / 2 + margin.top + margin.bottom) +
                ")"
            );
        } else {
          wrap.attr(
